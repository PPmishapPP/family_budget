...
public class MediaView extends VerticalLayout {
	...
	private void loadMediaForChat(Long chatId) {
		if (chatId != null) {
			List<MediaDto> mediaItems = mediaService.getMediaByChatId(chatId);
	...
	private HorizontalLayout createFilterPanel() {
		...
		moviesButton.addClickListener(e -> {
			if (selectedChat != null) {
				List<MediaDto> movies = mediaService.getMoviesByChatId(selectedChat.getId());
		...
		seriesButton.addClickListener(e -> {
			if (selectedChat != null) {
				List<MediaDto> series = mediaService.getSeriesByChatId(selectedChat.getId());
				mediaGrid.setItems(series);
				updateActiveFilter(seriesButton, allButton, moviesButton);
			}
		});

		allButton.setEnabled(selectedChat != null);
		moviesButton.setEnabled(selectedChat != null);
		seriesButton.setEnabled(selectedChat != null);
	...
	private void saveMediaItem(MediaDto item) {
		...
		mediaService.saveMedia(item, selectedChat.getChatId());
	}
	...
	private void createFooter() {
		...
		addButton.setEnabled(selectedChat != null);
