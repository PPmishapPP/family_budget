...
public class ForecastView extends Dialog {
	...
	public ForecastView(ForecastCalculator calculator, List<PeriodicChangeRuleDto> dtos, long chatId) {
	...
	private void calculateForecast() {
		...
		UI.getCurrent().access(() -> {
			try {
				// Выполняем расчет
				List<CalcItem> results = calculator.calc(dtos, chatId, targetDate);
